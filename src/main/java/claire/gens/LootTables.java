        ...
        LootTableEvents.MODIFY.register((key, tableBuilder, source, registries) -> {
            if (BuiltInLootTables.ABANDONED_MINESHAFT.equals(key) && source.isBuiltin()) {
                ...
                LootPool.Builder poolBuilder = LootPool.lootPool()
                        ...
                        .add(LootItem.lootTableItem(Items.ENCHANTED_BOOK).apply(new SetEnchantmentsFunction.Builder().withEnchantment(registries.lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(ResourceKey.create(Registries.ENCHANTMENT, Identifier.fromNamespaceAndPath(FloristEnchant.MOD_ID, "florist"))), ContextIntProviders.exactly(1))));
                tableBuilder.withPool(poolBuilder);
                ...
                LootPool.Builder poolBuilder = BuiltInLootTables.BASTION_TREASURE.equals(key) ?
                        LootPool.lootPool().setRolls((ContextIntProviders.exactly(1))).when(LootItemRandomChanceCondition.randomChance(0.25f)).add(LootItem.lootTableItem(ItemStuff.yag))
                        : LootPool.lootPool();
                LootItemFunction lootItemFunction = new LootItemFunction() {
                    @Override
                    public MapCodec<? extends LootItemFunction> codec() {
